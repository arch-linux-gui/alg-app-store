...
void HomeWidget::loadFeaturedPackages()
{
    ...
    for (auto& pkg : m_featuredPackages)
    {
        ...
        if (!repoInfo.name.isEmpty() && !repoInfo.repository.isEmpty())
        {
            ...
            {
                spdlog::debug("{}",
                              (QString("Package %1 found in %2 repository with version %3")
        ...
        else if (pkg.repository.toLower() == "aur")
        {
            ...
            spdlog::debug(
                "{}",
                (QString("Package %1 not found in enabled repositories, defaulting to AUR helper").arg(pkg.name))
                    .toStdString());
        }
    }
...
void HomeWidget::onPackageClicked(const PackageInfo& info)
{
    ...
    if (info.repository.toLower() == "aur")
    {
        ...
        connect(&aurHelper,
                ...
                [&fullInfo, &loop](const PackageInfo& aurInfo)
                {
                    fullInfo = aurInfo;
                    loop.quit();
                });
        ...
        connect(&aurHelper,
                &AurHelper::error,
                [&fullInfo, &info, &loop](const QString& error)
                {
