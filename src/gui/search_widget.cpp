    ...
    , m_aurHelper(std::make_unique<AurHelper>(this))
{
    ...
    connect(m_aurHelper.get(), &AurHelper::searchCompleted, this, &SearchWidget::onAurSearchCompleted);
    connect(m_aurHelper.get(), &AurHelper::error, this, &SearchWidget::onAurSearchError);
}
...
void SearchWidget::setupUi()
{
    ...
    connect(m_filterCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &SearchWidget::onFilterChanged);
...
void SearchWidget::onPackageClicked(const PackageInfo& info)
{
    ...
    if (info.repository.toLower() == "aur")
    {
        ...
        connect(aurHelper,
                &AurHelper::packageInfoReceived,
                [&](const PackageInfo& detailedInfo)
                {
        ...
        connect(aurHelper,
                ...
                [&](const QString& errorMsg)
                {
                    spdlog::warn(
                        "{}",
                        (QString("Failed to fetch AUR details for %1: %2").arg(info.name, errorMsg)).toStdString());
                    loop.quit();
                });
