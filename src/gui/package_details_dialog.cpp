    ...
    , m_logWidget(new QWidget(this))
{
    ...
    connect(&PackageManager::instance(),
            &PackageManager::operationStarted,
            this,
            &PackageDetailsDialog::onOperationStarted);
    connect(
        &PackageManager::instance(), &PackageManager::operationOutput, this, &PackageDetailsDialog::onOperationOutput);
    connect(&PackageManager::instance(),
            &PackageManager::operationCompleted,
            this,
            &PackageDetailsDialog::onOperationCompleted);
    connect(
        &PackageManager::instance(), &PackageManager::operationError, this, &PackageDetailsDialog::onOperationError);
}
...
void PackageDetailsDialog::setupUi()
{
    ...
    if (!m_info.upstreamUrl.isEmpty())
    {
        ...
        m_urlLabel->setText(QString("<a href='%1' style='color: #3b82f6;'>%1</a>").arg(m_info.upstreamUrl));
    ...
    if (isAUR)
    {
        ...
        connect(yayButton,
                &QPushButton::clicked,
                [yayButton, paruButton, commandText, this]()
                {
                    yayButton->setChecked(true);
    ...
    auto* noteLabel
        = new QLabel("Please ensure your system meets the minimum requirements before installation.", contentWidget);
    ...
    for (const QString& dir : desktopDirs)
    {
        ...
        for (const QString& pattern : patterns)
        {
            ...
            if (!matches.isEmpty())
            {
                ...
                spdlog::info("{}",
                             (QString("Found desktop file for %1: %2").arg(m_info.name, desktopFile)).toStdString());
                return desktopFile;
            }
        }
            ...
            for (const QString& desktopFileName : allDesktopFiles)
            {
                ...
                if (fileNameLower.contains(variant))
                {
                    ...
                    if (verifyDesktopFile(desktopFile, nameVariants + specialVariants + significantParts))
                    {
                        spdlog::info(
                            "{}",
                            (QString("Found desktop file for %1 via special match: %2").arg(m_info.name, desktopFile))
                                .toStdString());
                        return desktopFile;
                    }
                }
        ...
        for (const QString& desktopFileName : allDesktopFiles)
        {
            ...
            if (fileNameLower.contains(packageNameLower))
            {
                ...
                if (verifyDesktopFile(desktopFile, nameVariants + specialVariants + significantParts))
                {
                    spdlog::info(
                        "{}",
                        (QString("Found desktop file for %1 via full name match: %2").arg(m_info.name, desktopFile))
                            .toStdString());
                    return desktopFile;
                }
            }
            ...
            for (const QString& desktopFileName : allDesktopFiles)
            {
                ...
                if (wordBoundary.match(fileNameLower).hasMatch())
                {
                    ...
                    if (verifyDesktopFile(desktopFile, nameVariants + specialVariants + significantParts))
                    {
                        spdlog::info(
                            "{}",
                            (QString("Found desktop file for %1 via word match: %2").arg(m_info.name, desktopFile))
                                .toStdString());
                        return desktopFile;
                    }
                }
...
bool PackageDetailsDialog::verifyDesktopFile(const QString& desktopFilePath, const QStringList& nameVariants) const
{
...
void PackageDetailsDialog::launchApplication()
{
    ...
    if (desktopFile.isEmpty())
    {
        QMessageBox::warning(this,
                             ...
                             QString("Could not find a desktop file for %1.\n"
                                     ...
                                     "may need to be launched from the terminal.")
                                 .arg(m_info.name));
        return;
    }
    ...
    connect(process,
            ...
            this,
            [this, process, baseName](int exitCode, QProcess::ExitStatus exitStatus)
            {
                process->deleteLater();
