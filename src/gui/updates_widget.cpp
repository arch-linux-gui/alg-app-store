...
public:
        ...
        , m_info(info)
    {
        ...
        auto* versionLabel = new QLabel(QString("%1 → %2").arg(m_info.oldVersion, m_info.newVersion), this);
        ...
        connect(updateButton, &QPushButton::clicked, [this]() { emit updateRequested(m_info.name); });
    ...
    , m_logViewer(new QTextEdit(this))
{
    ...
    connect(&PackageManager::instance(), &PackageManager::operationStarted, this, &UpdatesWidget::onOperationStarted);
    connect(&PackageManager::instance(), &PackageManager::operationOutput, this, &UpdatesWidget::onOperationOutput);
    connect(
        &PackageManager::instance(), &PackageManager::operationCompleted, this, &UpdatesWidget::onOperationCompleted);
    connect(&PackageManager::instance(), &PackageManager::operationError, this, &UpdatesWidget::onOperationError);
}
        ...
        [this](std::stop_token stopToken)
        {
            ...
            QMetaObject::invokeMethod(
                ...
                [this, updates]()
                {
                    ...
                    {
                        m_statusLabel->hide();
                        m_countLabel->setText(QString("%1 updates available").arg(updates.size()));
...
void UpdatesWidget::displayUpdates(const QVector<UpdateInfo>& updates)
{
    ...
    for (const auto& update : updates)
    {
        ...
        connect(item, &UpdateItem::updateRequested, this, &UpdatesWidget::onUpdateSingle);
...
void UpdatesWidget::onUpdateAll()
{
    auto reply
        = QMessageBox::question(this,
                                "Update All",
                                QString("Are you sure you want to update all %1 packages?").arg(m_updates.size()),
