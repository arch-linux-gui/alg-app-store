    ...
    , m_filterTimer(new QTimer(this))
{
    ...
    connect(m_filterTimer, &QTimer::timeout, this, [this]() { filterPackages(m_filterInput->text()); });
...
void InstalledWidget::setupUi()
{
    ...
    connect(m_filterInput, &QLineEdit::textChanged, this, &InstalledWidget::onFilterTextChanged);
...
void InstalledWidget::filterPackages(const QString& query)
{
    ...
    else
    {
        ...
        {
            if (pkg.name.toLower().contains(lowerQuery) || pkg.description.toLower().contains(lowerQuery))
            {
    ...
    m_countLabel->setText(QString("%1 of %2 packages").arg(m_filteredPackages.size()).arg(m_allPackages.size()));
