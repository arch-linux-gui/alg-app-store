...
bool AlpmWrapper::initialize()
{
    ...
    if (!m_handle)
    {
        spdlog::error("{}", (QString("Failed to initialize ALPM: %1").arg(alpm_strerror(err))).toStdString());
        return false;
    }
    ...
    {
        alpm_db_t* db = alpm_register_syncdb(m_handle, repo.toStdString().c_str(), ALPM_SIG_USE_DEFAULT);
        if (!db)
        {
...
void AlpmWrapper::searchInDatabase(alpm_db_t* db, const QString& query, QVector<PackageInfo>& results)
{
    if (!db)
    {
        return;
...
QVector<UpdateInfo> AlpmWrapper::getAvailableUpdates()
{
    ...
    for (alpm_list_t* i = pkgs; i; i = i->next)
    {
        ...
        for (alpm_list_t* j = m_syncDbs; j; j = j->next)
        {
            ...
            if (syncPkg)
            {
                int cmp = alpm_pkg_vercmp(alpm_pkg_get_version(syncPkg), alpm_pkg_get_version(localPkg));
...
QStringList AlpmWrapper::getEnabledRepositories() const
{
    ...
    while (!in.atEnd())
    {
        ...
        if (line.startsWith("[") && line.endsWith("]") && !line.startsWith("#"))
        {
            ...
            if (repo != "options" && repo != "testing" && repo != "core-testing" && repo != "extra-testing"
