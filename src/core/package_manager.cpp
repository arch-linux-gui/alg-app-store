...
void PackageManager::uninstallPackage(const QString& packageName, const QString& repository)
{
    ...
    spdlog::info(
        "{}",
        (QString("Uninstalling package: %1 from %2").arg(packageName, repository.isEmpty() ? "default" : repository))
            .toStdString());
...
void PackageManager::updateAllPackages()
{
    ...
    QString command = QString("pkexec %1 -Syu --noconfirm").arg(getHelperName());
