...
private:
    ...
    std::unique_ptr< QProcess > m_process;  // Keep it for local AUR builds
    std::unique_ptr< QDBusInterface > m_daemonObj;
    mutable std::mutex m_mutex;

private slots:
    // Slots for local QProcess (AUR builds)
    void onProcessFinished( int exitCode, QProcess::ExitStatus exitStatus );
    void onProcessError( QProcess::ProcessError error );
    void onProcessOutput();

    // Slots for D-Bus daemon signals
    void onDaemonOperationOutput( const QString& output );
    void onDaemonOperationFinished( bool success, const QString& message );
};
